...
module.exports = class PgPersistence {
  ...
  async getPaginatedRecipes(limit, offset) {
    const RECIPES = "SELECT * FROM recipes WHERE username = $1 ORDER BY LOWER(recipe) LIMIT $2 OFFSET $3";
    const CATEGORIES = "SELECT * FROM categories WHERE username = $1";
    let resRecipes = await dbQuery(RECIPES, this.username, limit, offset);
  ...
  async recipesCount() {
    const TOT = "SELECT COUNT(*) FROM recipes WHERE username = $1";
    let result = await dbQuery(TOT, this.username);
