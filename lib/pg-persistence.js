...
module.exports = class PgPersistence {
  ...
  async getPaginatedCategories(limit, offset) {
    const CATEGORIES = "SELECT categories.*, count(recipes.recipe) AS tot_recipes " 
                +"FROM categories LEFT OUTER JOIN recipes ON categories.id = recipes.category_id "
                +"WHERE categories.username = $1 GROUP BY categories.id " 
