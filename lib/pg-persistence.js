...
module.exports = class PgPersistence {
  ...
  async getPaginatedCategories(limit, offset) {
    const CATEGORIES = "SELECT *, COUNT(*) OVER() AS total_count FROM categories WHERE username = $1 ORDER BY LOWER(category) LIMIT $2 OFFSET $3";
    let response  = await dbQuery(CATEGORIES, this.username, limit, offset);
    return response.rows;
  };

  async getPaginatedRecipes(catId, limit, offset) {
    const RECIPES_BY_CATEGORY = "SELECT *, COUNT(*) OVER() AS total_count FROM recipes WHERE category_id = $1 and username = $2 ORDER BY LOWER(recipe) ASC LIMIT $3 OFFSET $4";
  ...
  async deleteRecipe(id) {
    ...
    let response = await dbQuery(DELETE_RECIPE, id, this.username);
    return response.rowCount > 0; 
  };

  async getPaginationResult(totalCount, page, limit) {
